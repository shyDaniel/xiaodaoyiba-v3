...
**Fix.** Four edits, no new files:

1. `cp /tmp/judge_throw/t13000.png docs/screenshots/action.png` — replaces the offline mock with a live chromium-headless-shell capture from `scripts/validate-game-progression.mjs`. The captured frame is t=13s of a 3-player game (Bao48 + counter + random), surfaces the §C10 winner-picker overlay live (`你赢了！选个倒霉蛋` + counter / random target rows + `扒裤衩` / `咔嚓` buttons + `0.4秒后自动出手` countdown), shows the §C8 carved-wood BattleLog right-rail with R1.回合 + R1.平 entries narrating the real on-the-wire CN line `大家面面相觑，谁都没敢出招`, the §H2.1 iso 45° green-grass + dirt-path stage with three Kenney-Tiny-Town village houses, the §H2.5 left-edge mountain backdrop + drifting smoke wisps, and the §H2.4 carved-wood 石头 / 布 / 剪刀 HandPicker chips along the bottom. 1280×720, RGB, 97 KB.
2. `client/tests/render_action_static.gd` — header rewritten: line 1 now reads `compose a static design-mock action frame *without* a viewport (so it works in headless WSL2 where viewport rendering is blocked). Blits the SpriteAtlas house + character ImageTextures onto a hand-painted iso ground plane to produce a 1280×720 PNG used for offline pixel-coverage regression testing (see check_action_coverage.gd) — NOT for the docs gallery.` Final `img.save_png(...)` call now writes `docs/screenshots/action_mock.png` (the deterministic-anchor coverage-test target), not `docs/screenshots/action.png`. The S-401 `_blit_house()` comment that mentioned `action.png` updated to `action_mock.png`.
3. `client/tests/check_action_coverage.gd` — `path` and `abs_path` updated to read `action_mock.png` (the file the static mock now writes). Header gained an S-435 note explaining why we switched: the canonical `action.png` is a live capture with a different stage layout, so the deterministic-anchor coverage test reads the offline mock instead.
4. `README.md` — alt-text rewritten to describe the live R2.PREP frame (winner-picker overlay, 3 houses, BattleLog with real CN narration, HandPicker with 石头/布/剪刀 chips, mountain backdrop). Caption now reads `(live capture — chromium-headless-shell driving the Godot HTML5 build at localhost:5173 via scripts/validate-game-progression.mjs, R2.PREP frame at t=13s. The deterministic offline design mock now lives at docs/screenshots/action_mock.png, regenerated by client/tests/render_action_static.gd for the check_action_coverage.gd regression gate.)` Status section updated to match.

**Verification.**

- `git ls-files docs/screenshots/action.png` → present ✓.
- `head -3 client/tests/render_action_static.gd` → "compose a static design-mock action frame ... NOT for the docs gallery" — no longer claims to produce `docs/screenshots/action.png` ✓.
- `file docs/screenshots/action.png` → `PNG image data, 1280 x 720, 8-bit/color RGB, non-interlaced` (matches scripts/validate-game-progression.mjs viewport) ✓.
- Pixel-region eyeball of action.png: top-left phase banner `第2回合·准备`, right-rail BattleLog with `战报` / `R1.回合 第1回合，开打！` / `R1.平 大家面面相觑，谁都没敢出招` rows, center winner-picker dialog with 2 target rows + 2 action buttons + 0.4s auto-pick countdown, 2 named characters (Bao48 + random), 3 distinct Kenney houses, iso 45° green/dirt ground, mountain on left edge ✓.
- `godot --headless --path client --script res://tests/render_action_static.gd` → exit 0, writes `docs/screenshots/action_mock.png` 1280×720 RGBA 71 KB ✓.
- `godot --headless --path client --script res://tests/check_action_coverage.gd` → PASS, all 4 anchors house cov 50.3–93.1% / char cov 39.8–54.7%, all ≥ 30% threshold ✓.
- `pnpm test` → 90/90 green (79 shared + 11 server) ✓.
- `pnpm sim --players 4 --bots counter,random,iron,mirror --winner-strategy random-target+random-action --rounds 50 --seed 42` → tie_rate=0.260 (<0.30 ✓), max winner share 2/5=0.40 (<0.60 ✓), `PULL_OWN_PANTS_UP` fires R48 (`玩家不慌不忙，把裤衩穿了回去`) ✓, duration_ms=7 ✓.
- §I.0 procedural-draw grep → 0 hits in client/scripts/ (excluding tests/, particles/, 9slice, editor_only) ✓.

**What this closes.** Headline outstanding from iter 85→86→87→88→89 — `docs/screenshots/action.png` is now unambiguously a real live frame. The only remaining iter-89 carry-over items (winner-picker validator script, aesthetic-coverage script, mountain hue clusters) are independent threads.
