...
![xiaodaoyiba v3 — live R2.PREP frame: 第2回合·准备 phase banner top-left, winner-picker overlay (你赢了！选个倒霉蛋 with counter / random target rows + 扒裤衩 / 咔嚓 buttons + 0.4秒后自动出手 countdown), iso 45° green-grass + dirt-path stage with three Kenney-style village houses (Bao48 / random / a third on the back ridge), 战报 carved-wood BattleLog right-rail with R1.回合 + R1.平 entries narrating "大家面面相觑，谁都没敢出招", 石头 / 布 / 剪刀 HandPicker carved-wood chips along the bottom, mountain backdrop on the left edge](./docs/screenshots/action.png)
<sub>(live capture — chromium-headless-shell driving the Godot HTML5 build at `localhost:5173` via `scripts/validate-game-progression.mjs`, R2.PREP frame at t=13s. The deterministic offline design mock now lives at [`docs/screenshots/action_mock.png`](./docs/screenshots/action_mock.png), regenerated by `client/tests/render_action_static.gd` for the `check_action_coverage.gd` regression gate.)</sub>
...
Iterations 1–90 have shipped a playable end-to-end build. **The
screenshot above is a live capture** — chromium-headless-shell driving
the actual Godot HTML5 build at `localhost:5173` via
`scripts/validate-game-progression.mjs`, sampled at t=13s as a real
R2.PREP-tick frame with the C10 winner-picker overlay surfaced. The
deterministic offline design mock at `docs/screenshots/action_mock.png`
(composed by `client/tests/render_action_static.gd` — it blits the
SpriteAtlas house + character ImageTextures onto a hand-painted iso
ground plane and layers a phase banner / BattleLog / HandPicker via a
5×7 bitmap font + NotoColorEmoji.ttf glyphs) is retained as the
deterministic-anchor coverage-test target for `check_action_coverage.gd`,
not as the README hero image.
